...
## Удалённое тестирование через SSH Reverse Tunnel

Если устройство находится в локальной сети ноутбука, а сервер SmartYard — на удалённой машине, можно пробросить доступ через SSH reverse tunnel.
...
```bash
ssh -R 8080:192.168.1.3:80 user@server-ip -N
```

Параметры:
- `-R 8080:192.168.1.3:80` — проброс порта: сервер:8080 → устройство:80
...
```bash
# Проверка связи
curl -u admin:adminpass http://localhost:8080/api/device/status

# Открыть дверь
curl -X POST -u admin:adminpass http://localhost:8080/api/intercom/open_door

# Тестовый скрипт
php test_sputnikip.php http://localhost:8080 admin:adminpass
```

### В SmartYard

При добавлении устройства через туннель:
- **URL**: `http://localhost:8080`
- **Авторизация**: `admin:adminpass`
